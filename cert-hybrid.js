...
(async () => {
  console.log('=== 1) lê dump do poc (NopeCHA): authId + _csrf + token + cookies ===');
  const dump = JSON.parse(require('fs').readFileSync('/tmp/certtok.json', 'utf8'));
  const authId = dump.authId, csrf = dump.csrf, loginUrl = `https://sso.acesso.gov.br/login?client_id=comprasnet.gov.br&authorization_id=${dump.authId}`;
  for (const c of dump.cookies) { const h = c.domain.replace(/^\./, ''); (jar[h] = jar[h] || {})[c.name] = c.value; }
  global.__hcToken = dump.hcToken;
  console.log('  authId:', authId, '| _csrf:', csrf ? csrf.slice(0, 12) + '…' : 'NÃO', '| token NopeCHA:', global.__hcToken ? global.__hcToken.length + ' chars' : 'NÃO', '| cookies:', dump.cookies.length);
  if (!authId || !csrf || !global.__hcToken) { console.log('dump incompleto — abortando'); return; }

  console.log('\n=== 1.5) token do hCaptcha ===');
  let hcToken = global.__hcToken;
  if (hcToken) { console.log('  usando token NATIVO do browser (PAT coerente):', hcToken.length, 'chars'); }
  else {
    console.log('  browser não emitiu; caindo p/ 2Captcha (PAT pode não casar)...');
    const sol = await solveHcaptcha();
    if (!sol.ok) { console.log('  hCaptcha FALHOU:', sol.error); return; }
    hcToken = sol.token;
    console.log(`  ✓ token 2Captcha (${hcToken.length} chars) em ${sol.s}s`);
  }
  const sol = { token: hcToken };
