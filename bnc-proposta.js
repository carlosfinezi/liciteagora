// bnc-proposta.js
//
// Envio automático de PROPOSTA na BNC (bnccompras.com), server-side.
//
// Porta o padrão já validado no BLL (bll-proposta.js) para o BNC. Ambos são
// ASP.NET MVC com a mesma tela /Proposal/Proposal e form #jsonForm1.
// Spec de referência: memory project_bnc_connector_arch.
//
//   GET  /Proposal/Proposal?param1=[gkz]<processId>  → tela com itens + hidden
//   POST /Proposal/Proposal                           → SAVE (SEM reCAPTCHA)
//   - form-urlencoded, serialização do form DOM #jsonForm1
//   - autenticação só por cookie de sessão (capturado pelo Electron BNC)
//   - sucesso → resposta AJAX { Link: '/Base/DataResult?message=Proposta Salva' }
//     ou redirect pra /Base/DataResult (dependendo de como o MVC responde)
//
// Campos do payload (jsonForm1):
//   participantId        = [gkz]<token>      (hidden, estável por sessão)
//   IsMePortionAccepted  = True|False        (hidden — cota ME/EPP)
//   IsMe                 = True|False        (declaração ME/EPP)
//   fkRepresentant       = <GUID>            (representante legal — se a tela tiver)
//   processId            = [gkz]<token>      (hidden)
//   proposalItems[N].idBatchItem   = [gkz]<token por item>   (hidden)
//   proposalItems[N].ProposalValue = valor (decimal pt-BR, ex "11514,78")
//
// Fluxo (idêntico ao BLL):
...
const { bncGet, bncPost } = require('./bnc-client');
...
function parseAttrs(tag) {
  ...
  while ((m = re.exec(tag)) !== null) {
    ...
    // O BNC põe Name="participantId" ANTES de name="idParticipant" no mesmo input;
    // o jQuery .serialize() usa 'participantId'. Manter o primeiro casa com o browser.
...
// BNC pode não ter esse select; nesse caso retorna [] e não vai no payload.
...
async function carregarItens(db, urlOuToken) {
  ...
  const resp = await bncGet(db, url, { Accept: 'text/html,application/xhtml+xml,*/*' });
  ...
  if (!processId || !participantId || itens.length === 0) {
    const erro = new Error(
      'Página de proposta BNC não pôde ser parseada (processId/participantId/itens ausentes). ' +
      'Verifique a sessão (cookie) e o token param1.'
    );
    erro.code = 'BNC_PROPOSTA_PARSE';
...
// Decimal pt-BR com 2 casas. Aceita number (11514, 11514.78) ou string já BR.
...
function upsertProposta(db, row) {
  ...
  const existing = db.prepare('SELECT id, criadoEm, enviada FROM bnc_propostas WHERE processId = ?').get(row.processId);
  if (existing) {
    db.prepare(`UPDATE bnc_propostas SET
  ...
  const info = db.prepare(`INSERT INTO bnc_propostas
...
function extrairComprovante(html) {
  const m = /https?:\/\/[^"'\s]*(?:ReportPage\.aspx|ReportPage)[^"'\s]*/i.exec(html || '');
  return m ? m[0] : null;
}
...
// Resposta BNC pode vir como redirect pra DataResult (finalUrl) OU como
// envelope AJAX JSON { html, modal, Link, alerts } (X-Requested-With). Cobre os dois.
function pareceSucesso(resp) {
  const body = resp.body || '';
  const finalUrl = resp.finalUrl || '';

  // Envelope AJAX BNC (GetNewData): modal:'error' = falha; Link → DataResult.
  let env = null;
  try { env = JSON.parse(body); } catch { /* body não é JSON — segue nas heurísticas de texto */ }
  ...
  return (
    ...
    /Proposta\s*Salva|Proposta\s*enviada/i.test(body)
  );
}
...
//   compraId        (opcional)      — 'bnc:<processo>' p/ rastreio
...
async function enviarProposta(db, opts = {}) {
  ...
  const prev = db.prepare('SELECT * FROM bnc_propostas WHERE processId = ?').get(pagina.processId);
  ...
  if (docsFaltando.length) {
    ...
    erro.code = 'BNC_PROPOSTA_DOCS_FALTANDO';
    erro.docsFaltando = docsFaltando;
    throw erro;
  }

  // POST real
  const resp = await bncPost(db, PROPOSAL_PATH, payload, {
  ...
  if (!sucesso) {
    const erro = new Error(msgPortal ? `O BNC recusou a proposta: ${msgPortal}` : `POST /Proposal/Proposal não confirmou sucesso (HTTP ${resp.status}).`);
    erro.code = 'BNC_PROPOSTA_FALHA';
...
module.exports = {
  ...
  _resp: { pareceSucesso, extrairComprovante },
};
