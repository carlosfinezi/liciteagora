// bnc-signalr-client.js
//
// Cliente SignalR v2 (ASP.NET classic) para o hub `batchScreenHub` da BNC.
// Reusa cookie de sessão capturado pelo Electron BNC (config.bnc_session_cookie
// via bnc-client.js) — o mesmo cookie autentica negotiate, WebSocket e Ping.
//
...
//        { "C": "msgId", "M": [ { "H": "batchScreenHub", "M": "UpdateStatus", "A": [arg0, arg1, ...] } ] }
...
const {
  bncFetch, bncPost, BNC_BASE, UA,
  BncSessaoIndisponivelError, BncSessaoExpiradaError,
} = require('./bnc-client');
...
function readCookie(db) {
  const r = db.prepare("SELECT valor FROM config WHERE chave='bnc_session_cookie'").get();
  if (!r?.valor) throw new BncSessaoIndisponivelError();
  return r.valor;
}
...
async function negotiate({ db, processId, userId }) {
  ...
  const r = await bncFetch(db, '/signalr/negotiate?' + qs, {
    method: 'GET',
...
async function callStart({ db, connectionToken, processId, userId }) {
  ...
  const r = await bncFetch(db, '/signalr/start?' + qs, {
    method: 'GET',
    headers: { Accept: 'text/plain, */*; q=0.01' },
  });
  if (r.status !== 200) {
...
function createClient({ db, processId, userId, log }) {
  ...
  const _log = log || ((...a) => console.log('[bnc-signalr]', ...a));
  ...
  function buildWsUrl() {
    ...
    return 'wss://bnccompras.com/signalr/connect?' + qs;
  }
  ...
  function startPingTimer() {
    ...
    pingTimer = setInterval(async () => {
      ...
      try {
        const r = await bncPost(db, '/Notification/Ping?connectionId=' + encodeURIComponent(connectionId), null, {
