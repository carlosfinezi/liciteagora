...
/**
 * bll-session-service.js — Login BLL (bllcompras.com) SERVER-SIDE, sem Electron.
 *
 ...
 * resolvendo o reCAPTCHA in-page. Substitui o app Electron BLL:
 *
 *   FASE 1 (sessão)  — loga (teclado virtual + reCAPTCHA v2 invisible), extrai o
 *                      cookie BLL e entrega em POST /api/electron/bll/cookies
 *                      (o servidor grava bll_session_cookie/_at/_user/_perfil na
 *                      tabela config; bll-client.js/bll-proposta.js já consomem).
 *                      Keepalive via GET /Home/GetTimeNow; re-login ao expirar.
 *   FASE 2 (lances)  — relay de token reCAPTCHA v3: polla o servidor em
 *                      /api/electron/bll/captcha-pending, roda grecaptcha.execute
 *                      numa página BLL viva e devolve em /api/electron/bll/captcha-token
 *                      (porta de electron-standalone/portals/bll/captcha-relay.js).
 *
 * Login portado de electron-standalone/portals/bll/auto-login.js (o snippet dirige
 ...
 *   NOPECHA_KEY=xxx USE_EXTENSION=1 DELIVER=1 xvfb-run -a node bll-session-service.js
 *   # serviço (keepalive + relay infinito, systemd):
 *   NOPECHA_KEY=xxx USE_EXTENSION=1 SERVICE=1 DELIVER=1 RELAY=1 xvfb-run -a node bll-session-service.js
 *
...
const BLL_LOGIN_URL = `${BLL_BASE}/Home/Login`;
...
function readCreds(tenant) {
  ...
  const creds = {
    usuario: get('bll_usuario'),
    senha: get('bll_senha'),
    perfilPreferido: get('bll_perfil_preferido'),
    apiKey: get('api_key'),
  };
  db.close();
  if (!creds.usuario || !creds.senha) throw new Error('bll_usuario/bll_senha não configurados no tenant');
  return creds;
}
...
// mesmo do bll-client (redirect-login / 401 / 403 / HTML de login); qualquer outra
// coisa é viva (GetTimeNow, p.ex., dá 404 mesmo logado — não é sinal de morte).
function probeSession(cookieStr) {
  return new Promise((resolve) => {
    const u = new URL(`${BLL_BASE}/Participant/ProcessSearch?param1=0`);
    const req = https.request({
      ...
      let d = ''; res.on('data', (c) => d += c); res.on('end', () => {
        ...
        const morta = /\/(Home|Account)\/Login/i.test(loc) || res.statusCode === 401 || res.statusCode === 403 ||
...
// ─── snippet de auto-login (PORTA de portals/bll/auto-login.js) ─────────────
...
function buildAutoLoginSnippet({ email, senha, perfilPreferido }) {
  ...
  if (senhaLimpa.length !== 6) {
    return `window.__bllLoginResult = { ok: false, etapa: 'senha-invalida', error: 'Senha BLL deve ter 6 dígitos' };`;
  }
  ...
  return `(function () {
    window.__bllLoginResult = null;
    window.__bllLoginStatus = 'iniciando';
    const EMAIL = ${emailJs};
    const SENHA = ${senhaJs};
    const PERFIL_PREFERIDO = ${perfilPrefJs};
    function setResult(r) { window.__bllLoginResult = r; }
    function setStatus(s) { window.__bllLoginStatus = s; }
    ...
    async function aguardarResultadoLogin(maxMs = 90000) {
      ...
      while (Date.now() - inicio < maxMs) {
        ...
        if (errModal) return { ok: false, etapa: 'erro-bll', error: (errModal.innerText || '').trim().slice(0, 400) };
...
// ─── extração do cookie BLL (PORTA de cookie-sync.js) ───────────────────────
async function extractCookieStr(page) {
  let cookies = [];
  // page.cookies(url) já devolve os cookies de domain=.bllcompras.com aplicáveis a bllcompras.com.
  try { cookies = await page.cookies(BLL_BASE); } catch (e) {}
  const seen = new Set();
  const rel = [];
...
async function fullLogin(page, creds) {
  for (let tent = 1; tent <= LOGIN_MAX_TRIES; tent++) {
    ...
    await page.goto(BLL_LOGIN_URL, { waitUntil: 'domcontentloaded', timeout: 45000 }).catch((e) => log(`  goto: ${e.message}`));
    ...
    // destrói o contexto do snippet → __bllLoginResult some; por isso detectamos no
    ...
    while (Date.now() - t0 < 105000) {
      ...
      res = await page.evaluate(() => window.__bllLoginResult).catch(() => undefined);
      if (res && (res.ok || res.error)) break;
      const st = await page.evaluate(() => window.__bllLoginStatus).catch(() => null);
...
// entrega o cookie ao servidor (grava config.bll_session_cookie/_at/_user/_perfil).
async function deliverCookie(page, creds) {
  const cookie = await extractCookieStr(page);
  if (!cookie || !cookie.includes('=')) { log('  ⚠ sem cookie BLL pra entregar'); return { ok: false }; }
  if (!DELIVER) { log(`  (DELIVER=0) cookie capturado (${cookie.length} chars) — não entregue`); return { ok: true, cookie, delivered: false }; }
  const r = await serverReq('POST', '/api/electron/bll/cookies', { cookie, usuario: creds.usuario, perfil: creds.perfilPreferido }, creds.apiKey);
  log(`  entrega /api/electron/bll/cookies → HTTP ${r.status}`);
...
async function setupRelayPage(browser, creds) {
  ...
  // parkeia numa página BLL autenticada e garante grecaptcha v3 carregado.
  await rp.goto(BLL_BASE + '/Home/Index', { waitUntil: 'domcontentloaded', timeout: 20000 }).catch(() => {});
...
function startRelay(getRelayPage, creds) {
  ...
  async function loop() {
    ...
    inFlight = true;
    try {
      const r = await serverReq('GET', '/api/electron/bll/captcha-pending', null, creds.apiKey);
      const ped = r.body;
      if (ped && ped.id) {
        const rp = getRelayPage();
        log(`[relay] pedido id=${String(ped.id).slice(0, 8)} action=${ped.action}`);
        if (!rp) { await serverReq('POST', '/api/electron/bll/captcha-token', { id: ped.id, error: 'relay page indisponível' }, creds.apiKey); }
        else {
          try {
            const token = await executeRecaptcha(rp, ped.sitekey || SITEKEY, ped.action);
            log(`[relay] token len=${(token || '').length}`);
            await serverReq('POST', '/api/electron/bll/captcha-token', { id: ped.id, token }, creds.apiKey);
          } catch (e) {
            log(`[relay] erro grecaptcha: ${e.message}`);
            await serverReq('POST', '/api/electron/bll/captcha-token', { id: ped.id, error: e.message || String(e) }, creds.apiKey);
          }
        }
      }
...
// ────────────────────────────────────────────────────────────────────────────
async function main() {
  log(`bll-session-service — tenant=${TENANT} service=${SERVICE} relay=${RELAY} deliver=${DELIVER} probe=${PROBE_SEC}s`);
