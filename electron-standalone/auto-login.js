...
function create({ ctx, utils }) {
  ...
  // Limpa os cookies da sessão do webview Comprasnet (partition dedicada;
  // BNC usa sesBnc separada, não é afetada). Necessário antes de um re-login:
  // quando a sessão SSO expira, o cookie residual faz o loginPortal.asp
  // redirecionar para intro.htm — página sem o botão "Entrar com Gov.br" —
  // e o re-login falha em loop ("Botão Entrar não encontrado"). Limpar os
  // cookies força o loginPortal.asp a exibir o formulário de login de novo.
  async function limparSessaoComprasnet() {
    try {
      const wv = getWV();
      await wv.session.clearStorageData({ storages: ['cookies'] });
      ctx.log('Sessão Comprasnet limpa (cookies) — forçando login limpo');
    } catch (e) {
  ...
  async function autoLogin(cpf, senha, opts = {}) {
    ...
    try {
      // 0a. Re-login: limpar cookies da sessão morta ANTES de navegar, senão
      // o loginPortal.asp quica para intro.htm e não há botão Gov.br pra clicar.
      if (opts.clearSession) await limparSessaoComprasnet();
      ...
      if (!senhaEncontrada) {
        const url = wv.getURL();
        ctx.log(`Campo de senha não apareceu após 40s. URL: ${url}`);
  ...
  async function autoLoginFromDB() {
    ...
    if (result.success) {
      ...
      ctx.log(`Auto-login do banco: FALHA — ${result.message}`);
      ctx.log('Tentando novamente em 60s...');
      await sleep(60000);
      if (ctx.getState() !== 'logged_in') {
        // 1ª tentativa falhou → sessão pode estar suja; limpar antes do retry.
        const retry = await autoLogin(creds.cpf, creds.senha, { clearSession: true });
        if (retry.success) {
          ctx.log('Auto-login retry: SUCESSO');
        } else {
          ctx.log(`Auto-login retry: FALHA — ${retry.message}. Aguardando login manual.`);
        }
      }
    }
  }
