...
/**
 * portals/bll/auto-login.js — Auto-login no BLL (bllcompras.com).
 *
 * Porta do portals/bnc/auto-login.js: a BLL roda a MESMA plataforma do BNC
 * (login em /Home/Login com teclado virtual `soma()` + reCAPTCHA invisible +
 * `doLogin()`; campos #Email/#Password; modal de escolha de perfil). Por isso
 * o snippet é praticamente idêntico — muda só host, URL e os nomes das vars de
 * resultado (__bllLoginResult / __bllLoginStatus).
 *
 * O snippet preenche window.__bllLoginResult / __bllLoginStatus que o main
 * process lê via executeJavaScript a cada 1s (até 45s timeout).
 *
 * Uso:
 *   const al = require('./auto-login').create({ ctx, serverBridge, getCookieSync });
 ...
 * getCookieSync().forceSync() é chamado após login OK (sync de cookies).
 */

const BLL_LOGIN_URL = 'https://bllcompras.com/Home/Login';
const POLL_MAX_MS = 45000;

// ─── Snippet builder (porta de portals/bnc/auto-login.js) ──────────────────
function buildAutoLoginSnippet({ email, senha, perfilPreferido }) {
  ...
  return `(function () {
    ...
    async function tentarSelecionarPerfil() {
      ...
      // Renderiza cada perfil como uma linha com texto + um <a> só de ícone:
      //   <tr>
      ...
      window.__bllProfileCandidates = candidatos.map(c => ({
      ...
      try {
        ...
        return {
          ...
          candidatos: window.__bllProfileCandidates.map(c => c.txt),
    ...
    async function aguardarResultadoLogin(maxMs = 30000) {
      ...
      while (Date.now() - inicio < maxMs) {
        ...
        if (errModal) {
          ...
          return { ok: false, etapa: 'erro-bll', error: txt };
        }
        ...
        if (perfilModal && /perfil/i.test(perfilModal.textContent || '')) {
          if (!perfilSelecionado) {
            ...
            if (sel && sel.ok) {
              ...
              try { localStorage.setItem('__bll_last_profile_pick', JSON.stringify(infoSelecao)); } catch (_) {}
...
// ─── Trigger orchestrator (porta de portals/bnc/auto-login.js) ─────────────
function create({ ctx, serverBridge, getCookieSync }) {
  ...
  async function trigger({ manual = false } = {}) {
    const log = ctx.log;
    if (inProgress) {
      log('Autologin BLL já em andamento — ignorando');
      return;
    }
    const wv = ctx.getWebview();
    if (!wv || wv.isDestroyed()) {
      log('Autologin BLL: webview não pronto');
      return;
    }
    ...
    try {
      ...
      log(`Autologin BLL: falha credenciais — ${e.message}`);
    ...
    if (!/\/Home\/Login/i.test(url)) {
      log(`Navegando para ${BLL_LOGIN_URL} (estava em ${url})`);
      await wv.loadURL(BLL_LOGIN_URL);
      await new Promise(r => setTimeout(r, 1500));
    }
    ...
    try {
      ...
      log(`Autologin BLL: erro ao injetar snippet — ${e.message}`);
    ...
    while (Date.now() - inicio < POLL_MAX_MS) {
      ...
      let res;
      try {
        res = await wv.executeJavaScript('window.__bllLoginResult', true);
      } catch (e) {
        log(`Polling BLL: ${e.message}`);
        continue;
      }
      if (res === null || res === undefined) {
        try {
          const s = await wv.executeJavaScript('window.__bllLoginStatus', true);
      ...
      inProgress = false;
      if (res.ok) {
        log(`Autologin BLL OK: ${res.etapa} ${res.url || ''}`);
        if (res.selecao) {
          log(`[BLL perfil] estrategia=${res.selecao.estrategia} escolhido="${res.selecao.escolhido}" alvoNorm="${res.selecao.alvoNorm || ''}" candidatos=${JSON.stringify(res.selecao.candidatos || [])}`);
        }
        ...
        log(`Autologin BLL FALHA (${res.etapa}): ${res.error}`);
        setStatus(`Falha (${res.etapa}): ${res.error}`, 'err');
        try { await serverBridge.reportError('bll-autologin', new Error(`${res.etapa}: ${res.error}`)); } catch {}
      }
      return;
    }

    inProgress = false;
    log('Autologin BLL: polling timeout');
