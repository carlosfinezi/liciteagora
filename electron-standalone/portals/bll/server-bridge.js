...
/**
 * portals/bll/server-bridge.js — HTTP client pra endpoints BLL do servidor
 * LiciteAgora. Clone de portals/bnc/server-bridge.js (mesma mecânica HTTP),
 * trocando apenas os paths /api/electron/bll/* e o User-Agent.
 *
 * Endpoints usados:
 *   GET  /api/electron/bll/credentials  → { usuario, senha, perfilPreferido? }
 *   POST /api/electron/bll/cookies      → { cookie, usuario?, perfil? }
 *   GET  /api/electron/bll/session      → { configurado, idadeMs, usuario, perfil, expirando }
...
function request(method, pathOrUrl, body, { timeout = 15000, requireAuth = true } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const headers = {
      ...
      'User-Agent': 'LiciteAgora-BLL/1.0',
    };
...
async function fetchCredentials() {
  const r = await request('GET', '/api/electron/bll/credentials');
  if (r.status !== 200) {
...
async function sendCookies({ cookie, usuario, perfil }) {
  const r = await request('POST', '/api/electron/bll/cookies', { cookie, usuario, perfil });
  if (r.status !== 200) throw new Error(`cookies HTTP ${r.status}: ${JSON.stringify(r.body)}`);
  return r.body;
}

async function getSessionStatus() {
  const r = await request('GET', '/api/electron/bll/session');
...
async function reportError(ctx, err) {
  try {
    ...
    _log(`[bridge BLL] reportError falhou: ${e.message}`);
  }
}
